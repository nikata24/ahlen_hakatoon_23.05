...
// Retrieve search query and coordinates
...
try {
    ...
    if (!empty($googleApiKey)) {
        // Take the first word of the query to prevent Google Places API from returning ZERO_RESULTS
        $search_query = $query;
        ...
        $url = "https://maps.googleapis.com/maps/api/place/textsearch/json?query=" . urlencode($search_query . " in Ahlen Germany") . "&location=" . $latVal . "," . $lngVal . "&radius=3500&key=" . urlencode($googleApiKey);
        ...
        if ($text_data && isset($text_data['results'])) {
            ...
            foreach ($limited as $rawPlace) {
                ...
                // Get detailed details for each place
                ...
                if ($detail_data && isset($detail_data['result'])) {
                    ...
                    $placeItem = [
                        ...
                        'coordinates' => [
                            'lat' => isset($detail['geometry']['location']['lat']) ? (float)$detail['geometry']['location']['lat'] : $latVal,
                            'lng' => isset($detail['geometry']['location']['lng']) ? (float)$detail['geometry']['location']['lng'] : $lngVal
                        ],
    ...
    $db_path = dirname(__DIR__) . '/places_db.json';
